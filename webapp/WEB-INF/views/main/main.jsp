...
<c:forEach var="vo" items="${list }" varStatus="status" begin="0">
	...
	<input type="hidden" id="location${status.index+1 }"
		value="${vo.location}">
	<input type="hidden" id="address${status.index+1 }"
		value="${vo.address}">
	<%-- ${vo.no}
			${vo.location}
			${vo.address} --%>
...
<section>
	...
	<script type="text/javascript"
		src="//apis.daum.net/maps/maps3.js?apikey=9a98e6a57e71d0677b9b9649676f151b&libraries=services"></script>
	<script>
		var mapContainer = document.getElementById('map'), // 지도를 표시할 div 
		mapOption = {

			center : new daum.maps.LatLng(37.49499176831797, 127.02729942476266), // 지도의 중심좌표
			level : 1, // 지도의 확대 레벨
		// 지도종류
		};
		...
		function displayPlaces(places) {
			...
			for (var i = 0; i < places.length; i++) {
				...
				var placePosition = new daum.maps.LatLng(places[i].latitude,
						places[i].longitude), itemEl = getListItem(0, places[0]); // 검색 결과 항목 Element를 생성합니다
...
<aside>
	...
	<c:forEach var="vo" items="${list }" varStatus="status" begin="0">
		...
		<c:forEach var="vopic" items="${picList }" varStatus="status"
			begin="0">
