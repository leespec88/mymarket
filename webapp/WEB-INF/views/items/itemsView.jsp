	...
	<div class="container" id="itemview">
		<div class="row">
			<div class="col-sm-5 largeImg">
				<img class="img-thumbnail" id="" src="${fileList[0].IMAGE}" alt="Chania" width="500" height="400">
			</div>
			<div class="col-sm-1" id="smallimage">
					<c:forEach var="file" items="${fileList}">
						<div class=row>
							<img class="img-thumbnail" id="${file.IMAGE}" src="${file.IMAGE}" alt="Chania" width="52" height="40" onmouseover="javascript:setImage('${file.IMAGE}');">	
						</div>
					</c:forEach>
			</div>
			<div class="col-sm-6">
				...
				<div class="well">
					<div class="row">
						...
						<div class="col-sm-8">
							<p><a href="">${userVo.name}</a></p>
							<p><a href="/items/userItemList/${userVo.no}">등록 아이템 : ${regItemCnt}</a></p>
							<p><a href="#"><span class="glyphicon glyphicon-phone-alt"></span> ${userVo.phone}</a></p>
						</div>
					</div>
				</div>
			</div>
		...
		<div class="well">
			<div class="row">
				<div class="col-sm-6">
					<h5><strong>상세설명</strong></h5>
					<textarea readonly="readonly" class="form-control" rows="5" id="comment">${itemVo.content}</textarea>
					<script>
				...
				<div class="col-sm-6">
					<c:if test="${authUser.no != userVo.no }">
						<p><img src="/assets/images/img_side_clean_market_titletext.png"></p>
						<div class="well">
							<h5><strong>사람들의 눈살을 찌푸리게 하는,거래금지 아이템은 자제해주세요!</strong></h5>
						...
						<div class="well">
							<h5><strong>에티켓을 지키면 꼭 돌아오는 법! 비매너 행위는 자제해주세요~</strong></h5>
					...
					<c:if test="${authUser.no == userVo.no }">
						...
						<div class="well">
							<form action="/items/updateSellState/${itemVo.no}" method="post">
								<label class="radio-inline">
							    	<input type="radio" name="sellState" value="판매중">판매중
							    </label>
							    <label class="radio-inline">
							    	<input type="radio" name="sellState" value="예약중">예약중
							    </label>
							    <label class="radio-inline">
							    	<input type="radio" name="sellState" value="판매완료">판매완료
							    </label>
						...
						<p>
							<a href="#"><button type="button" class="btn btn-primary"> 재등록 </button></a>
							<a href="#"><button type="button" class="btn btn-success"> 상단노출 </button></a>
							<a href="/items/updateItem/${itemVo.no}"><button type="button" class="btn btn-warning">아이템수정</button></a>
							<a href="/items/itemDelete/${itemVo.no}"><button type="button" class="btn btn-danger" onclick="del_event();">아이템삭제</button></a>
						</p>
					 </c:if>
				</div>
			</div>
